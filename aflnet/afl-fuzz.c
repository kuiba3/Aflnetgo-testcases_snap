...
static u8* describe_op(u8 hnb) {
  ...
  if (syncing_party) {
    ...
    sprintf(ret, "sync_%s,src:%06u", syncing_party, syncing_case);
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  switch (fault) {
    ...
    case FAULT_TMOUT:
      ...
      fn = alloc_printf("%s/replayable-hangs/id_%06llu,%s", out_dir,
...
keep_as_crash:
      ...
      fn = alloc_printf("%s/replayable-crashes/id_%06llu,%llu,sig_%02u", out_dir,
                        unique_crashes, get_cur_time() - start_time,
                        kill_signal);
...
static void maybe_delete_out_dir(void) {
  ...
  /* Okay, let's get the ball rolling! First, we need to get rid of the entries
     in <out_dir>/.synced/.../id_*, if any are present. */
  ...
  /* Then, get rid of the .state subdirectory itself (should be empty by now)
     and everything matching <out_dir>/queue/id_*. */
  ...
  /* All right, let's do <out_dir>/replayable-crashes/id_* and <out_dir>/replayable-hangs/id_*. */
...
#define SP20    SP10 SP10
  ...
  if (cpu_core_count) {
    ...
    if (cpu_aff >= 0) {
      ...
      SAYF(SP10 cGRA "[cpu%03u_%s%3u%%" cGRA "]\r" cRST,
           MIN(cpu_aff, 999), cpu_color,
           MIN(cur_utilization, 999));

    } else {

      SAYF(SP10 cGRA "   [cpu_%s%3u%%" cGRA "]\r" cRST,
